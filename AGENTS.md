...
本仓库是一个**安全任务技能路由包**（逆向工程 / 渗透测试 / 安全分析）。[RULES.md](RULES.md) 是行为链唯一真相源。

## 路由

用户任务命中安全/逆向关键词时：

1. [skills/MASTER-ROUTING.md](skills/MASTER-ROUTING.md) 或平台对应入口 → PRIMARY：
...
2. 歧义时读 [skills/routing.md](skills/routing.md) 全矩阵（三轴：目标类型 / 用户意图 / 工具链）
3. 路由规则唯一事实源：[skills/config/routing.json](skills/config/routing.json)（改路由只改这里）
...
- 证据链：[skills/ops/evidence-finding-path.md](skills/ops/evidence-finding-path.md)；角色：[skills/ops/role-map.md](skills/ops/role-map.md)

## 首次运行

[skills/tool-index.md](skills/tool-index.md) 是 gitignored 的生成文件，首次使用前按平台运行：
...
缺工具 → 使用同平台 bootstrap：Windows [skills/scripts/bootstrap-reverse.ps1](skills/scripts/bootstrap-reverse.ps1)；Linux / macOS [skills/scripts/bootstrap-reverse.sh](skills/scripts/bootstrap-reverse.sh)；Kali [kali/scripts/bootstrap-reverse.sh](kali/scripts/bootstrap-reverse.sh)（清单能力，禁止猜路径）。
...
- [skills/INDEX.md](skills/INDEX.md) 由 `extract-summaries.ps1` 从全部 `SKILL.md` 动态生成，不硬编码客户端或模块数量。
