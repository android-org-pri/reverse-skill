...
2. `NOW`: **创建/更新 case**（[../scripts/case-init.ps1](../scripts/case-init.ps1)）并完成 `scope.md`（[../ops/scope-contract.md](../ops/scope-contract.md)）；`auth.status!=granted` 禁止 ACT
3. `NOW`: 以 **lead** 角色规划阶段（[../ops/role-map.md](../ops/role-map.md)），写入 specialist_roles
4. `NEXT`: 读取 [../tool-index.md](../tool-index.md)，校验工具可用性和实际路径
5. `NEXT`: 缺工具时调用 bootstrap，不要猜路径
6. `ACT`: 按 [references/lifecycle-checklist.md](references/lifecycle-checklist.md) 过阶段门闩；每阶段更新 `timeline.md` + `workitems.md`（[../ops/timeline-workitem.md](../ops/timeline-workitem.md)）；发现提升为 Evidence/Finding
